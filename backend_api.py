...
class MissionResult(BaseModel):
    ...
    delta_v_required_km_s: Optional[float] = None
    flight_time_days: Optional[float] = None
    fuel_consumed_percent: Optional[float] = None
...
async def simulate_mission(mission: MissionParameters):
    ...
    try:
        ...
        intercept_date_str = mission.intercept_date.replace('Z', '+00:00')
        intercept_date = datetime.fromisoformat(intercept_date_str).replace(tzinfo=None)  # Remove timezone info
        ...
        # Format response with REAL mission parameters
        mission_result = MissionResult(
            success=result['success'],
            target_object=mission.target_object,
            intercept_date=mission.intercept_date,
            data_sources=result.get('data_sources', []),
            intercept_time=result['intercept_time'].isoformat() if 'intercept_time' in result and result['intercept_time'] else None,
            ...
            # REAL MISSION PARAMETERS - NO DEMO DATA
            delta_v_required_km_s=result.get('delta_v_required_km_s'),
            flight_time_days=result.get('flight_time_days'), 
            fuel_consumed_percent=result.get('fuel_consumed_percent'),
            success_probability=result.get('success_probability'),
            data_captured=result.get('data_captured', [])
        )
